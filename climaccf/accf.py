...
import xarray as xr
from climaccf.contrail import *
from climaccf.database import *
from climaccf.emission_indices import *
from climaccf.weather_store import get_bound_indexes
...
class GeTaCCFs(object):
    ...
    def get_accfs(self, **problem_config):
        ...
        if self.aCCF_Version == 'V1.0':
            self.aCCF_CO2 =  7.48 * 1e-16 * np.ones(self.t.shape)*self.sf['CO2']/self.base['CO2']   # P-ATR20-CO2 [K/kg(fuel)]
        elif self.aCCF_Version  == 'V1.0A':
            self.aCCF_CO2 =  7.48 * 1e-16 * np.ones(self.t.shape)*self.sf['CO2']/self.eg['CO2'] 
        else:
