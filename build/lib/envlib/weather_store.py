...
class WeatherStore(WeatherStore_):
    ...
    def __init__(self, weather_data, weather_data_sur=None, flipud='auto', **weather_config):
        ...
        inf_variables = extract_data_variables(weather_data, weather_data_sur, verbose=True)
        inf_coordinates, weather_data, weather_data_sur = extract_coordinates(weather_data, inf_variables,
                                                                              weather_data_sur)
        ...
        for i_, tag in enumerate(self.variable_names):
            ...
            self.values[tag_] = A
            if self.cfg['save_as_xr']:
                self.var_xr[tag_] = (tuple(self.coordinate_names), A)
        if self.cfg['predecimate']:
